...
export const Exercise: React.FC<ExerciseProps> = ({subjectId, exercise}) => {
    ...
    const getImage = (path: string | File): string => {
        if (typeof path === 'string') {
            return path;
        }
        throw new Error('Invalid path: expected a string or a File object');
    };
        ...
        <section className="flex flex-col items-center h-[40rem] relative w-[60rem]">
            ...
            <ExerciseProvider oldExercise={exercise}>
                ...
                <section className="w-full flex flex-col items-center mt-8">
                    {exercise.questionImageUrl &&
                        <Image src={getImage(exercise.questionImageUrl)} width={100} height={100} alt={""}/>}
