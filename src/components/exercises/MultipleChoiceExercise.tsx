...
import { updateExerciseById } from "@/actions/exerciseActions";
...
export const MultipleChoiceExercise: React.FC<MultipleChoiceExerciseProps> = ({
  ...
  const [editedExercise, setEditedExercise] =
    useState<ExerciseMultipleChoice>(exercise);
        ...
        <div className="w-full h-1/2">
          ...
          {edit ? (
            <div className="flex flex-col">
              ...
              />
              <h1 className="text-2xl mt-4">Choices</h1>
              {editedExercise.choices.map((choice, index) => (
                <input
                  key={index}
                  className="resize-none border border-bgColor_neutral_emphasis pl-1 w-full h-fit rounded-md mt-2"
                  value={choice}
                  onChange={(e) => {
                    const newChoices = [...editedExercise.choices];
            ...
            <>
              ...
              {exercise.choices.map((choice, index) => (
                <p key={index} className="text-xl">{choice}</p>
              ))}
            </>
          )}
        </div>
