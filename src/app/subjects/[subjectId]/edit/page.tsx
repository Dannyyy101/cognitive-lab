            ...
            <IsUserAdmin>
                ...
                {showDeletePopUp &&
                    <PopUpView handlePopUpClose={() => setShowDeletePopUp(false)}>
                        <DeleteView handleDelete={handleDeleteSubject} correctAnswer={subject.name}>
                        </DeleteView></PopUpView>}
            </IsUserAdmin>
        </main>
    );
}
...
const DisplaySubjects = ({subject}: { subject: SubjectDTO }) => {
    ...
    return (
        <div className="p-4 rounded-md border-borderColor_default border w-80 h-40 relative m-2">
            ...
            <Link href={`/subjects/${parent.id}/children/${subject.id}/edit`}
                ...
                <button
                    className="text-bgColor_default absolute bottom-4 right-4 w-32 h-10 rounded-md bg-bgColor_inverse font-semibold">
                    Bearbeiten
                </button>
            </Link>
        </div>
