...
export default function Page() {
    ...
    const handleGetAllExercises = (): ExerciseBaseDTO[] =>{
        const temp:ExerciseBaseDTO[][] = []
        subject.children.map((child) => temp.push(child.exercises))
        console.log(temp.flat())
        return temp.flat()
    }

    const [exercises, _setExercises] = useState<ExerciseBaseDTO[]>(handleGetAllExercises())
    ...
    const isAnswerCorrect = () => {
        return checkAnswer(
            userAnswer,
            (exercises[exerciseIndex].answer[0] as ExerciseTextComponent)
                .content,
        );
    };
        ...
        <main className="w-screen h-screen flex justify-center items-center relative">
            ...
            <section className="w-2/3 min-h-96 flex border-l-4 border-l-bgColor_accent_emphasis border border-borderColor_default rounded-md flex-col relative">
                ...
                <div className="h-20 relative w-full border-t border-t-borderColor_default mt-2 p-4">
                    {exercises[exerciseIndex].documentationUrl && (
                        <Link
                            href={exercises[exerciseIndex].documentationUrl}
