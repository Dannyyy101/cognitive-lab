...
import { difficulties, mapSubjectRatingFromText } from '@/utils/subjectFunctions';
...
const CreateSubjectView = () => {
    ...
    const handleUpdateSubject = async () => {
        ...
        await Promise.all(
            oldSubject.exercises.map(async (oldExercise) => {
                const index = subject.exercises.findIndex((e) => e.id === oldExercise.id);
                if (index === -1) {
    ...
    const handleAddExercise = () => {
        setSubject({
            ...subject,
            exercises: [...subject.exercises, getDefaultExerciseByType(newExerciseType)],
        });
    };
        ...
        <main className="w-screen mt-32 flex justify-center items-center flex-col h-fit">
            <IsUserAdmin>
                ...
                <section className="relative w-10/12 rounded-md border-borderColor_default border p-4 mt-4 flex flex-col">
                    ...
                    <p className="text-fgColor_disabled text-sm">Enter the basic information about your exercise</p>
                    ...
                    <select
                        onChange={(e) =>
                            setSubject({ ...subject, difficulty: mapSubjectRatingFromText(e.target.value) })
                        }
                        className="max-w-96 h-8 text-bgColor_default border border-borderColor_default pl-1 rounded-md"
                    >
                        {difficulties.map((difficulty) => (
                            <option className="text-borderColor_default" key={difficulty}>
                                {difficulty}
                            </option>
                        ))}
                    </select>
