...
export const ExerciseExplorer = ({
    ...
    return (
        <section className="flex flex-col w-32 overflow-y-auto mr-4">
            {subject.exercises.map((_exercise, index) => (
                <div
                    className={`my-1 flex items-center h-8 rounded-md ${focusedExerciseIndex === index ? 'bg-bgColor_accent_emphasis text-white' : 'bg-white text-black'}`}
                    key={index}
                >
                    <button className={`focus:bg-black w-20`} onClick={() => handleFocusExercise(index)}>
                        Frage {index + 1}
                    </button>
